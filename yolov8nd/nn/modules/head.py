...
class NDetectAux(nn.Module):
    ...
    def __init__(self, nc=80,ch=()):
        ...
        self.cv1 = nn.ModuleList(nn.Sequential(Conv(x, x, 3, 1), Conv(x, c1, 1, 1), Conv(c1, c2, 3, 1)) for x in ch[::2])
        self.cv2 = nn.ModuleList(nn.Sequential(nn.Conv2d(c2, c2, 1, 1)) for x in ch[::2])
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, x, 3, 1), Conv(x, c1, 1, 1), Conv(c1, c2, 3, 1)) for x in ch[1::2])
